...
mysql_query("SET NAMES 'utf8'");
	...
	if($xml) { 
			...
			if ($sport->Name=="Tennis" ){echo "<br>Sportart:Tennis";
				foreach ($sport->children() as $category) {
						//echo "category: ";
					if($category->Name=="ATP" || $category->Name=="WTA" || $category->Name=="Challenge" || $category->Name=="ITF Men" || $category->Name=="ITF Women" || $category->Name=="Italy" || $category->Name=="International"){
						...
						$LigaId=$tournament->attributes();
							if($LigaId['BetradarTournamentId']=="31853" || $LigaId['BetradarTournamentId']=="31854" || $LigaId['BetradarTournamentId']!="10771" || $LigaId['BetradarTournamentId']=="10772" || $LigaId['BetradarTournamentId']=="41" || $LigaId['BetradarTournamentId']=="23" || $LigaId['BetradarTournamentId']=="10909" || $LigaId['BetradarTournamentId']=="36" || $LigaId['BetradarTournamentId']=="1" || $LigaId['BetradarTournamentId']=="16" || $LigaId['BetradarTournamentId']=="8343" || $LigaId['BetradarTournamentId']=="43" || $LigaId['BetradarTournamentId']=="150" || $LigaId['BetradarTournamentId']=="62" || $LigaId['BetradarTournamentId']=="33" || $LigaId['BetradarTournamentId']=="86"|| $LigaId['BetradarTournamentId']=="3955"|| $LigaId['BetradarTournamentId']=="3956"|| $LigaId['BetradarTournamentId']=="3957"|| $LigaId['BetradarTournamentId']=="3958"|| $LigaId['BetradarTournamentId']=="3959"|| $LigaId['BetradarTournamentId']=="3960"|| $LigaId['BetradarTournamentId']=="3948" ){
								...
								if($match->MatchDate[0]!="" && $match->Team1->Name[0]!="" && $match->Team2->Name[0]!=""){
									...
									//echo $match->Team1->Name[0]."</td><td class=\"ls_team2\">".$match->Team2->Name[0]."</td><td class=\"ls_status_result\">";							
										...
										echo "<br>Start: ".$match->MatchDate[0];
											...
											if($status=="Ended" || $status=="1st set" || $status=="2nd set" || $status=="3rd set" || $status=="4th set" || $status=="5th set" || $status=="Pause"){	
...
//																					$pointattr=$satzergebnis->Point[$countpoints]->attributes();
								...
								if($LigaId=="31853"){$league="Atlanta";}
								...
								else if ($LigaId=="23"){$league="Champions League";}
								else if ($LigaId=="10909") {$league="Europa League";}
								...
								else if ($LigaId=="3955") {$league="WM Gruppe B";}
								else if ($LigaId=="3956") {$league="WM Gruppe C";}
								else if ($LigaId=="3957") {$league="WM Gruppe D";}
								else if ($LigaId=="3958") {$league="WM Gruppe E";}
								else if ($LigaId=="3959") {$league="WM Gruppe F";}
								...
								if($teamnamehome=="" || $teamnameaway=="" || $matchId2db<10){}
									...
									//mysql_query("INSERT INTO `ls_matches_tennis` (matchid, league, tournament, matchdate, matchstatus, hometeam, awayteam, scorehome, scoreaway, p1_scorehome, p1_scoreaway, p2_scorehome, p2_scoreaway, p3_scorehome, p3_scoreaway, p4_scorehome, p4_scoreaway, p5_scorehome, p5_scoreaway, uniqueTeamHome, uniqueTeamAway, lastgoalteam, servingplayer, pointhome, pointaway, lastchangeby) VALUES ('$matchId2db', '$country', '$league', '$m_timestamp','$status','$teamnamehome','$teamnameaway','$scorehome','$scoreaway', '$p1_home', '$p1_away', '$p2_home', '$p2_away', '$p3_home', '$p3_away', '$p4_home', '$p4_away', '$p5_home', '$p5_away', 'uniqueTeamHome', 'uniqueTeamAway', '$lastscoring', '$actserving', '$pointhome','$pointaway','main')");
										//$logname="main_xml2db.log";
										//$datei=fopen($logname,"a");
										//fputs($datei,"$eintragen\n\n");
										//fclose($datei);
										
									$checkrows = mysql_query("SELECT `matchid` FROM `ls_matches_tennis` where `matchid` = '$matchId2db' Limit 1");
									$num_rows = mysql_num_rows($checkrows);
									$row = mysql_fetch_array($checkrows);
									
									if ($num_rows<1){
									mysql_query("INSERT INTO `ls_matches_tennis` (matchid, league, tournament, matchdate, matchstatus, hometeam, awayteam, scorehome, scoreaway, p1_scorehome, p1_scoreaway, p2_scorehome, p2_scoreaway, p3_scorehome, p3_scoreaway, p4_scorehome, p4_scoreaway, p5_scorehome, p5_scoreaway, uniqueTeamHome, uniqueTeamAway, lastgoalteam, servingplayer, pointhome, pointaway, lastchangeby, xmltime) VALUES ('$matchId2db', '$country', '$league', '$m_timestamp','$status','$teamnamehome','$teamnameaway','$scorehome','$scoreaway', '$p1_home', '$p1_away', '$p2_home', '$p2_away', '$p3_home', '$p3_away', '$p4_home', '$p4_away', '$p5_home', '$p5_away','$uniqueTeamIdHome2db','$uniqueTeamIdAway2db', '$lastscoring', '$actserving', '$pointhome','$pointaway','delta', $xmltime) ON DUPLICATE KEY 
									UPDATE `matchid` = VALUES(matchid), `league` = VALUES(league), `tournament` = VALUES(tournament), `matchdate` = VALUES(matchdate), `matchstatus` = VALUES(matchstatus), `hometeam` = VALUES(hometeam), `awayteam` = VALUES(awayteam), `scorehome` = VALUES(scorehome),`scoreaway` = VALUES(scoreaway),`p1_scorehome` = VALUES(p1_scorehome),`p1_scoreaway` = VALUES(p1_scoreaway),`p2_scorehome` = VALUES(p2_scorehome),`p2_scoreaway` = VALUES(p2_scoreaway),`p3_scorehome` = VALUES(p3_scorehome),`p3_scoreaway` = VALUES(p3_scoreaway),`p4_scorehome` = VALUES(p4_scorehome),`p4_scoreaway` = VALUES(p4_scoreaway),`p5_scorehome` = VALUES(p5_scorehome),`p5_scoreaway` = VALUES(p5_scoreaway),`uniqueTeamHome` = VALUES(uniqueTeamHome),`uniqueTeamAway` = VALUES(uniqueTeamAway),`lastgoalteam` = VALUES(lastgoalteam),`servingplayer` = VALUES(servingplayer),`pointhome` = VALUES(pointhome),`pointaway` = VALUES(pointaway),`lastchangeby` = VALUES(lastchangeby),`xmltime` = VALUES(xmltime)");
									$num_rows=0;																																																																																																																																																										 																																			
									echo "test".$status;
									}
