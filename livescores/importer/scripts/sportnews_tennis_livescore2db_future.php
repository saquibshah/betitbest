...
$xmlFile='/opt/users/www/betitbest/xmls/sportradar/lct/livescore_multicast_betitbest_lct_4250_future.xml';
...
/*
	...
	if($doc){
		...
		$query=mysql_query("SELECT counter FROM sportnews_livescores_tennis LIMIT 1");
...
//																					$pointattr=$satzergebnis->Point[$countpoints]->attributes();
				...
				if ($status=="1st set"){
					...
					//$sessionTime=$sessionTime+1;
							...
							$lstChngQuery=mysql_query("SELECT lastchangeby FROM sportnews_livescores_tennis WHERE matchid='$matchId'");
							...
							$live_match_query = mysql_query("SELECT * FROM `sportnews_livescores_tennis` where `matchstatus` = '1st set' || `matchstatus` = '2nd set' || `matchstatus` = '3rd set' || `matchstatus` = '4th set' || `matchstatus` = '5th set'");
								
							while($row = mysql_fetch_array($live_match_query)){
								$match_id = $row['matchid'];
								if ($match_id!=$matchId2db){
									echo "LCA match ID:".$match_id."<br/>";
									//$status_update_query = mysql_query("UPDATE sportnews_livescores_tennis SET matchstatus='LCA' WHERE matchid='$match_id'");
									}
								}
							//end LCA check
							*/
							if($teamnamehome=="" || $teamnameaway=="" || $matchId2db<10){}
								...
								if($counter>$counterValueFromDB){
									mysql_query("INSERT INTO `sportnews_livescores_tennis` (matchid, leagueid, categoryid, uniquetournamentid, country, league, matchdate, matchstatus, hometeam, awayteam, scorehome, scoreaway, p1_scorehome, p1_scoreaway, p2_scorehome, p2_scoreaway, p3_scorehome, p3_scoreaway, p4_scorehome, p4_scoreaway, p5_scorehome, p5_scoreaway, uniqueTeamHome, uniqueTeamAway, lastgoalteam, servingplayer, pointhome, pointaway, winner, lastchangeby, xmltime, counter) VALUES ('$matchId2db', '$LigaId', '$CategoryId','$UniqueTournamentId', '$category', '$tournamentname', '$m_timestamp','$status','$teamnamehome','$teamnameaway','$scorehome','$scoreaway', '$p1_home', '$p1_away', '$p2_home', '$p2_away', '$p3_home', '$p3_away', '$p4_home', '$p4_away', '$p5_home', '$p5_away','$uniqueTeamIdHome2db','$uniqueTeamIdAway2db', '$lastscoring', '$actserving', '$pointhome', '$pointaway', '$winner', 'future', $xmltime,'$counter') ON DUPLICATE KEY 
									UPDATE `matchid` = VALUES(matchid),`leagueid` = VALUES(leagueid), `categoryid` = VALUES(categoryid), `uniquetournamentid` = VALUES(uniquetournamentid), `league` = VALUES(league), `country` = VALUES(country), `matchdate` = VALUES(matchdate), `matchstatus` = VALUES(matchstatus), `hometeam` = VALUES(hometeam), `awayteam` = VALUES(awayteam), `scorehome` = VALUES(scorehome),`scoreaway` = VALUES(scoreaway),`p1_scorehome` = VALUES(p1_scorehome),`p1_scoreaway` = VALUES(p1_scoreaway),`p2_scorehome` = VALUES(p2_scorehome),`p2_scoreaway` = VALUES(p2_scoreaway),`p3_scorehome` = VALUES(p3_scorehome),`p3_scoreaway` = VALUES(p3_scoreaway),`p4_scorehome` = VALUES(p4_scorehome),`p4_scoreaway` = VALUES(p4_scoreaway),`p5_scorehome` = VALUES(p5_scorehome),`p5_scoreaway` = VALUES(p5_scoreaway),`uniqueTeamHome` = VALUES(uniqueTeamHome),`uniqueTeamAway` = VALUES(uniqueTeamAway),`lastgoalteam` = VALUES(lastgoalteam),`servingplayer` = VALUES(servingplayer),`pointhome` = VALUES(pointhome),`pointaway` = VALUES(pointaway),`winner` = VALUES(winner),`lastchangeby` = VALUES(lastchangeby),`xmltime` = VALUES(xmltime),`counter` = VALUES(counter)");
									echo mysql_errno(),
									"<br>",
									mysql_error();									
									...
									echo "data not updated from xml<br>(Cause: Counter<=DbCounter)<br>";
								}							
								$checkrows = mysql_query("SELECT `matchid` FROM `sportnews_livescores_tennis` where `matchid` = '$matchId2db' AND `lastchangeby` = 'delta' Limit 1");
								echo mysql_errno(),
								"<br>",
								mysql_error();
