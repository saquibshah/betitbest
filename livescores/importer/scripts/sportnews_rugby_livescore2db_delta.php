						...
						foreach ($category->children() as $tournament) {
							...
							foreach ($tournament->children() as $match){
								...
								if($teamnamehome=="" || $teamnameaway=="" || $matchId2db<10){}
									...
									if ($num_rows<1){
									mysql_query("INSERT INTO `sportnews_livescores_rugby` (matchid, leagueid, categoryid, uniquetournamentid, country,league,matchdate,matchstatus,hometeam,awayteam,lastperioddate,scorehome,scoreaway,lastgoaltime, uniqueTeamHome, uniqueTeamAway, lastgoalteam, scoremin, scoredby, createtime,lastchangeby,xmltime) VALUES ('$matchId2db', '$LigaId', '$CategoryId', '$UniqueTournamentId2db', '$country', '$tournamentname', '$m_timestamp','$status','$teamnamehome','$teamnameaway','$p_timestamp','$scorehome','$scoreaway','$lastgoaltime','$uniqueTeamIdHome2db','$uniqueTeamIdAway2db','$lastgoalteam', '$minscored','$lastgoalscoredby','$jetzt','delta','$xmltime') ON DUPLICATE KEY
