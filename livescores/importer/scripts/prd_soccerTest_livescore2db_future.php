		...
		foreach( $sports as $sport ){
			...
			if ($name=="Soccer" ){
				...
				foreach ($catagoryNames as $categories){
					...
					if($category=="Germany" || $category=="International Champions Cup" || $category=="Cyprus" || $category=="Spain" || $category=="International Clubs" || $category=="England" || $category=="Turkey" || $category=="Italy" || $category=="International" || $category=="Argentina" || $category=="Australia" || $category=="Austria" || $category=="Belgium" || $category=="Denmark" || $category=="France" || $category=="Germany Amateur" || $category=="Greece" || $category=="Holland" || $category=="Mexico" || $category=="Portugal" || $category=="Russia" || $category=="Scotland" || $category=="Switzerland" || $category=="Ukraine" || $category=="USA" ){
						$country=$category;
								...
								foreach ($matches as $match){
									...
									if(($match->getElementsByTagName( 'MatchDate' )->item(0)->nodeValue)!="" && ($homeTeam->item(0)->nodeValue)!="" && ($awayTeam->item(0)->nodeValue)!=""){
										...
										if($teamnamehome=="" || $teamnameaway=="" || $matchId2db<10){}
											...
											$checkrows = mysql_query("SELECT `matchid` FROM `ls_matches_soccerTest` where `matchid` = '$matchId2db' AND '$xmltime' > `xmltime` Limit 1");
											//echo mysql_errno(),
											//"<br>",
											mysql_error();
											...
											mysql_query("INSERT INTO `ls_matches_soccerTest` (matchid, leagueid,categoryid,uniquetournamentid,country,league,matchdate,matchstatus,hometeam,awayteam,lastperioddate,scorehome,scoreaway,lastgoaltime, uniqueTeamHome, uniqueTeamAway, lastgoalteam, scoremin, scoredby, createtime,lastchangeby,xmltime,counter) VALUES ('$matchId2db','$LigaId','$CategoryId','$UniqueTournamentId','$country', '$league', '$m_timestamp','$status','$teamnamehome','$teamnameaway','$p_timestamp','$scorehome','$scoreaway','$lastgoaltime','$uniqueTeamIdHome2db','$uniqueTeamIdAway2db','$lastgoalteam', '$minscored','$lastgoalscoredby','$jetzt','future','$xmltime','$counter') ON DUPLICATE KEY 
