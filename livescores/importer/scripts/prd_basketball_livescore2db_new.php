...
/*if (file_exists($xmlFile)) { 
	...
	//$dbh = new PDO('mysql:host=localhost;dbname=db1029865-bib;charset=utf8', 'db1029865-sosu', 'bib_db_sosu');
					...
					if($category=="Germany" || $category=="Lithuania" || $category=="Sweden" || $category=="International" || $category=="Austria" || $category=="USA" || $category=="Poland"){
						...
						foreach ($tournaments as $tournament) {
							...
							if($LigaId=="154" || $LigaId=="177" || $LigaId=="1537" || $LigaId=="1820" || $LigaId=="737" || $LigaId=="4805" || $LigaId=="1550" || $LigaId=="9154" || $LigaId=="3191" || $LigaId=="2451"){
								...
								$matches = $tournament->getElementsByTagName( 'Match' );												foreach ($matches as $match){
													$p1_home=0;
													$p1_away=0;
													$p2_home=0;
													$p2_away=0;
...
//HIER SCORING/SERVING ÄNDERN																		
								...
								if($teamnamehome=="" || $teamnameaway=="" || $matchId2db<10){}
									...
									$checkrows = mysql_query("SELECT `matchid` FROM `ls_matches_basketball_test` where `matchid` = '$matchId2db' Limit 1");
									echo mysql_errno(),
									"<br>",
									mysql_error();
