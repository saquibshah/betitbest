...
//																					$pointattr=$satzergebnis->Point[$countpoints]->attributes();
				...
				if ($status=="1st set"){
					...
					//$sessionTime=$sessionTime+1;
							...
							$lstChngQuery=mysql_query("SELECT lastchangeby FROM ls_matches_tennis_new WHERE matchid='$matchId'");
							...
							if($teamnamehome=="" || $teamnameaway=="" || $matchId2db<10){}
								...
								if($counter>$counterValueFromDB){
									mysql_query("INSERT INTO `ls_matches_tennis_new` (matchid,leagueid, country, league, matchdate, matchstatus, hometeam, awayteam, scorehome, scoreaway, p1_scorehome, p1_scoreaway, p2_scorehome, p2_scoreaway, p3_scorehome, p3_scoreaway, p4_scorehome, p4_scoreaway, p5_scorehome, p5_scoreaway, uniqueTeamHome, uniqueTeamAway, lastgoalteam, servingplayer, winner, pointhome, pointaway, lastchangeby, xmltime, counter) VALUES ('$matchId2db', '$LigaId', '$category', '$tournamentname', '$m_timestamp','$status','$teamnamehome','$teamnameaway','$scorehome','$scoreaway', '$p1_home', '$p1_away', '$p2_home', '$p2_away', '$p3_home', '$p3_away', '$p4_home', '$p4_away', '$p5_home', '$p5_away','$uniqueTeamIdHome2db','$uniqueTeamIdAway2db', '$lastscoring', '$actserving', '$winner', '$pointhome','$pointaway','main', $xmltime,'$counter') ON DUPLICATE KEY
