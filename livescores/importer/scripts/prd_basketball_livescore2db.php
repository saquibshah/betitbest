				...
				foreach ($sport->children() as $category) {
					...
					if($category->Name=="Germany" || $category->Name=="Lithuania" || $category->Name=="Sweden" || $category->Name=="International" || $category->Name=="Austria" || $category->Name=="USA" || $category->Name=="Poland" ){
						...
						$LigaId=$tournament->attributes()['BetradarTournamentId'];
							if($LigaId=="154" || $LigaId=="6107" || $LigaId=="177" || $LigaId=="1820" || $LigaId=="1537" || $LigaId=="737" || $LigaId=="4805" || $LigaId=="1550" || $LigaId=="9154" || $LigaId=="3191" || $LigaId=="2451" || $LigaId=="10561" ){
...
//HIER SCORING/SERVING �NDERN
								...
								$live_match_query = mysql_query("SELECT * FROM `ls_matches_basketball` where `matchstatus` = '1st quarter' || `matchstatus` = '2nd quarter' || `matchstatus` = '3rd quarter' || `matchstatus` = '4th quarter' || `matchstatus` = 'Overtime'");
										
										while($row = mysql_fetch_array($live_match_query)){
											$match_id = $row['matchid'];
											if ($match_id!=$matchId2db){
												echo "LCA match ID:".$match_id."<br/>";
												//$status_update_query = mysql_query("UPDATE ls_matches_basketball SET matchstatus='LCA' WHERE matchid='$match_id'");
											}
										}
										//end LCA check
								...
								if($teamnamehome=="" || $teamnameaway=="" || $matchId2db<10){}
									...
									mysql_query("INSERT INTO `ls_matches_basketball` (matchid, leagueid, country, league, matchdate, matchstatus, hometeam, awayteam, uniqueTeamHome, uniqueTeamAway, scorehome, scoreaway, q1_scorehome, q1_scoreaway, q2_scorehome, q2_scoreaway, q3_scorehome, q3_scoreaway, q4_scorehome, q4_scoreaway, ot_scorehome, ot_scoreaway, winner, winnername, lastteamscored, createtime, lastchangeby, xmltime) VALUES ('$matchId2db', '$LigaId', '$country', '$league', '$m_timestamp','$status','$teamnamehome','$teamnameaway','$uniqueTeamIdHome2db','$uniqueTeamIdAway2db','$scorehome','$scoreaway', '$p1_home', '$p1_away', '$p2_home', '$p2_away', '$p3_home', '$p3_away', '$p4_home', '$p4_away', '$ot_home', '$ot_away', '$winner', '$winnername', '$lastscoring', '$jetzt', 'main', '$xmltime') ON DUPLICATE KEY
									...
									$checkrows = mysql_query("SELECT `matchid` FROM `ls_matches_basketball` where `matchid` = '$matchId2db' Limit 1");
									echo mysql_errno(),
									"<br>",
									mysql_error();
