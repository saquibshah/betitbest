...
//			foreach($xml->Tournament as $sport) {
			...
			if ($sport->Name=="Ice Hockey" ){echo "<br>Sportart:Ice Hockey";
				foreach ($sport->children() as $category) {
						//echo "category: ";
					if($category->Name=="Germany" || $category->Name=="Czech" || $category->Name=="Sweden" || $category->Name=="Russia" || $category->Name=="USA" || $category->Name=="International"){){
						...
						$LigaId=$tournament->attributes()['BetradarTournamentId'];
							if($LigaId=="109" || $LigaId=="93" || $LigaId=="673" || $LigaId=="115" || $LigaId=="142" || $LigaId=="6795" || $LigaId=="10560" || $LigaId=="1981" || $LigaId=="1982" || $LigaId=="388"  ){
...
//HIER SCORING/SERVING �NDERN
								...
								else if ($LigaId=="388"){$league="World Championship, Final Round";}
								else if ($LigaId=="1981"){$league="World Championship, Group A";}
								else if ($LigaId=="1982"){$league="World Championship, Group B";}
								...
								if($teamnamehome=="" || $teamnameaway=="" || $matchId2db<10){}
									...
									$checkrows = mysql_query("SELECT `matchid` FROM `ls_matches_ice_hockey` where `matchid` = '$matchId2db' Limit 1");
									echo mysql_errno(),
									"<br>",
									mysql_error();
									...
									if ($num_rows<1){
									mysql_query("INSERT INTO `ls_matches_ice_hockey` (matchid, leagueid, country, league, matchdate, matchstatus, hometeam, awayteam, uniqueTeamHome, uniqueTeamAway, scorehome, scoreaway, q1_scorehome, q1_scoreaway, q2_scorehome, q2_scoreaway, q3_scorehome, q3_scoreaway, ot_scorehome, ot_scoreaway, winner, winnername, lastteamscored, createtime, lastchangeby, xmltime) VALUES ('$matchId2db', '$LigaId', '$country', '$league', '$m_timestamp','$status','$teamnamehome','$teamnameaway','$uniqueTeamIdHome2db','$uniqueTeamIdAway2db','$scorehome','$scoreaway', '$p1_home', '$p1_away', '$p2_home', '$p2_away', '$p3_home', '$p3_away', '$ot_home', '$ot_away', '$winner', '$winnername', '$lastscoring', '$jetzt', 'delta', '$xmltime') ON DUPLICATE KEY
