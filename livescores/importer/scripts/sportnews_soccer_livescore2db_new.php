...
//Setting table name and live Status
$lcaProcess->setTable('sportnews_livescores_soccer');
$lcaProcess->setLiveStatus(array('1st half', '2nd half', 'Halftime', '1st extra', '2nd extra'));
	...
	mysql_query($query);
	  ...
	  //if($BetradarLivescoreData->generatedAt[0]!="") { Den ganzen Block bitte nur direkt unter der if($xml) Schleife legen. Es werden keine anderen Schelifen benötigt
		...
		foreach( $sports as $sport ){
						...
						foreach ($tournaments as $tournament) {
							...
							if($LigaId!=""){
								...
								foreach ($matches as $match){
									...
									//This match is updating
                                    $lcaProcess->removeFromLCA($matchId);
									...
									if(($match->getElementsByTagName( 'MatchDate' )->item(0)->nodeValue)!="" && ($homeTeam->item(0)->nodeValue)!="" && ($awayTeam->item(0)->nodeValue)!=""){
										...
										if($teamnamehome=="" || $teamnameaway=="" || $matchId2db<10){}
											...
											if($counter>$matchIdcounterValueFromDB && $matchIdcounterValueFromDB!=""){
												...
												echo "<b>data not updated from xml<br>(Cause: xmlCounter=DbCounter)</b><br>";
											}
											...
											if ($num_rows<1 && $counter>$matchIdcounterValueFromDB && $status !="Ended"){
			...
			//Mark all matches that wasn't updated as LCA
			$lcaProcess->markAsLCAs();
		}
		mysql_close();
	}
?>