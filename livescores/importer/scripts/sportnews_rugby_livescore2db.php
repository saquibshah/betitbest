						...
						foreach ($category->children() as $tournament) {
							...
							foreach ($tournament->children() as $match){
								...
								//to check matches for Live Coverage Abandoned
										$live_match_query = mysql_query("SELECT * FROM `sportnews_livescores_rugby` where `matchstatus` = '1st half' || `matchstatus` = '2nd half' || `matchstatus` = 'Halftime'  || `matchstatus` = '1st extra'  || `matchstatus` = '2nd extra' ");
										
										while($row = mysql_fetch_array($live_match_query)){
											$match_id = $row['matchid'];
											if ($match_id!=$matchId2db){
												echo "LCA match ID:".$match_id."<br/>";
												//$status_update_query = mysql_query("UPDATE sportnews_livescores_rugby SET matchstatus='LCA' WHERE matchid='$match_id'");
											}
										}
										//end LCA check
								*/
								...
								if($teamnamehome=="" || $teamnameaway=="" || $matchId2db<10){}
									...
									mysql_query("INSERT INTO `sportnews_livescores_rugby` (matchid, leagueid, categoryid, uniquetournamentid, country,league,matchdate,matchstatus,hometeam,awayteam,lastperioddate,scorehome,scoreaway,lastgoaltime, uniqueTeamHome, uniqueTeamAway, lastgoalteam, scoremin, scoredby, createtime,lastchangeby,xmltime) VALUES ('$matchId2db', '$LigaId', '$CategoryId', '$UniqueTournamentId2db', '$country', '$tournamentname', '$m_timestamp','$status','$teamnamehome','$teamnameaway','$p_timestamp','$scorehome','$scoreaway','$lastgoaltime','$uniqueTeamIdHome2db','$uniqueTeamIdAway2db','$lastgoalteam', '$minscored','$lastgoalscoredby','$jetzt','main','$xmltime') ON DUPLICATE KEY
