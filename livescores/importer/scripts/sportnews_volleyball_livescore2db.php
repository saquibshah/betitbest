...
//																					$pointattr=$satzergebnis->Point[$countpoints]->attributes();
								...
								$live_match_query = mysql_query("SELECT * FROM `sportnews_livescores_volleyball` where `matchstatus` = '1st set' || `matchstatus` = '2nd set' || `matchstatus` = '3rd set' || `matchstatus` = '4th set' || `matchstatus` = '5th set'");
									
								while($row = mysql_fetch_array($live_match_query)){
									$match_id = $row['matchid'];
									if ($match_id!=$matchId2db){
										echo "LCA match ID:".$match_id."<br/>";
										//$status_update_query = mysql_query("UPDATE sportnews_livescores_volleyball SET matchstatus='LCA' WHERE matchid='$match_id'");
										}
									}
								//end LCA check
								*/
