					...
					foreach ($tournaments as $tournament) {
						...
						foreach ($matches as $match){									
								...
								if($teamnamehome=="" || $teamnameaway=="" || $matchId2db<10){}
									...
									mysql_query("INSERT INTO `sportnews_livescores_tennis` (matchid, leagueid,categoryid,uniquetournamentid,country,league,matchdate,matchstatus,hometeam,awayteam,winner,uniqueTeamHome, uniqueTeamAway,lastchangeby,xmltime) VALUES ('$matchId2db', '$LigaId', '$CategoryId','$UniqueTournamentId', '$category', '$tournamentname', '$m_timestamp','$status','$teamnamehome','$teamnameaway','$winner','$uniqueTeamIdHome2db','$uniqueTeamIdAway2db','future','$xmltime') ON DUPLICATE KEY 
									UPDATE `matchid` = VALUES(matchid),`leagueid` = VALUES(leagueid), `categoryid` = VALUES(categoryid), `uniquetournamentid` = VALUES(uniquetournamentid), `league` = VALUES(league), `country` = VALUES(country), `matchdate` = VALUES(matchdate), `matchstatus` = VALUES(matchstatus), `hometeam` = VALUES(hometeam), `awayteam` = VALUES(awayteam), `winner` = VALUES(winner), `uniqueTeamHome` = VALUES(uniqueTeamHome),`uniqueTeamAway` = VALUES(uniqueTeamAway),`lastchangeby` = VALUES(lastchangeby),`xmltime` = VALUES(xmltime)");
									echo mysql_errno(),
									"<br>",
									mysql_error();		
									}
