				...
				foreach ($sport->children() as $category) {
					...
					if($category->Name=="Germany" || $category->Name=="Greece" || $category->Name=="International" || $category->Name=="Italy" || $category->Name=="Poland"){
						...
						$LigaId=$tournament->attributes()['BetradarTournamentId'];
							if($LigaId=="833" || $LigaId=="2741" || $LigaId=="3781" || $LigaId=="1247" || $LigaId=="36539" || $LigaId=="36538" || $LigaId=="5892" || $LigaId=="5893" || $LigaId=="5894" || $LigaId=="5895" || $LigaId=="6408" || $LigaId=="34345" || $LigaId=="37146" || $LigaId=="38652" || $LigaId=="13999" || $LigaId=="4985" || $LigaId=="4986" || $LigaId=="4987" || $LigaId=="4988" || $LigaId=="5197" || $LigaId=="13939" || $LigaId=="13941" || $LigaId=="5155" || $LigaId=="4976" || $LigaId=="4977" || $LigaId=="4978" || $LigaId=="4979" || $LigaId=="5113" || $LigaId=="5114" || $LigaId=="38909" || $LigaId=="38910" || $LigaId=="11074" || $LigaId=="14380" || $LigaId=="14378" || $LigaId=="14379" || $LigaId=="14377" || $LigaId=="14375" || $LigaId=="14376" || $LigaId=="10014" || $LigaId=="10015" || $LigaId=="6397" || $LigaId=="6127" || $LigaId=="6128" || $LigaId=="35001" || $LigaId=="35002" || $LigaId=="35003" || $LigaId=="5946" || $LigaId=="5947" || $LigaId=="5948" || $LigaId=="5979" || $LigaId=="6793" || $LigaId=="6794" || $LigaId=="8352" || $LigaId=="845" || $LigaId=="1702" || $LigaId=="2846" || $LigaId=="3018" || $LigaId=="3019" || $LigaId=="7044" || $LigaId=="7528" || $LigaId=="10078" || $LigaId=="9306" || $LigaId=="9307" || $LigaId=="9308" || $LigaId=="9309" || $LigaId=="9310" || $LigaId=="21406" || $LigaId=="9300"|| $LigaId=="9301"|| $LigaId=="9302"|| $LigaId=="9303"|| $LigaId=="9304"|| $LigaId=="9305"|| $LigaId=="10079"|| $LigaId=="21405" || $LigaId=="10422" || $LigaId=="15953" || $LigaId=="10539" || $LigaId=="10407" || $LigaId=="33569" || $LigaId=="23663" || $LigaId=="10534" || $LigaId=="33571" || $LigaId=="33572" || $LigaId=="42076" || $LigaId=="15564"){
...
//																					$pointattr=$satzergebnis->Point[$countpoints]->attributes();
								...
								$live_match_query = mysql_query("SELECT * FROM `ls_matches_volleyball` where `matchstatus` = '1st set' || `matchstatus` = '2nd set' || `matchstatus` = '3rd set' || `matchstatus` = '4th set' || `matchstatus` = '5th set'");
										
										while($row = mysql_fetch_array($live_match_query)){
											$match_id = $row['matchid'];
											if ($match_id!=$matchId2db){
												echo "LCA match ID:".$match_id."<br/>";
												//$status_update_query = mysql_query("UPDATE ls_matches_volleyball SET matchstatus='LCA' WHERE matchid='$match_id'");
											}
										}
										//end LCA check
								...
								if($teamnamehome=="" || $teamnameaway=="" || $matchId2db<10){}
									...
									mysql_query("INSERT INTO `ls_matches_volleyball` (matchid, leagueid, country, league, matchdate, matchstatus, hometeam, awayteam, scorehome, scoreaway, p1_scorehome, p1_scoreaway, p2_scorehome, p2_scoreaway, p3_scorehome, p3_scoreaway, p4_scorehome, p4_scoreaway, p5_scorehome, p5_scoreaway, uniqueTeamHome, uniqueTeamAway, lastgoalteam, servingplayer, pointhome, pointaway, winner, winnername, lastchangeby, xmltime) VALUES ('$matchId2db', '$LigaId', '$country', '$league', '$m_timestamp','$status','$teamnamehome','$teamnameaway','$scorehome','$scoreaway', '$p1_home', '$p1_away', '$p2_home', '$p2_away', '$p3_home', '$p3_away', '$p4_home', '$p4_away', '$p5_home', '$p5_away','$uniqueTeamIdHome2db','$uniqueTeamIdAway2db','$lastgoalteam', '$actserving', '$pointhome','$pointaway', '$winner', '$winnername', 'main', $xmltime) ON DUPLICATE KEY
									UPDATE `leagueid` = VALUES(leagueid), `matchid` = VALUES(matchid), `country` = VALUES(country),`league` = VALUES(country),  `matchdate` = VALUES(matchdate), `matchstatus` = VALUES(matchstatus), `hometeam` = VALUES(hometeam), `awayteam` = VALUES(awayteam), `scorehome` = VALUES(scorehome),`scoreaway` = VALUES(scoreaway),`p1_scorehome` = VALUES(p1_scorehome),`p1_scoreaway` = VALUES(p1_scoreaway),`p2_scorehome` = VALUES(p2_scorehome),`p2_scoreaway` = VALUES(p2_scoreaway),`p3_scorehome` = VALUES(p3_scorehome),`p3_scoreaway` = VALUES(p3_scoreaway),`p4_scorehome` = VALUES(p4_scorehome),`p4_scoreaway` = VALUES(p4_scoreaway),`p5_scorehome` = VALUES(p5_scorehome),`p5_scoreaway` = VALUES(p5_scoreaway),`uniqueTeamHome` = VALUES(uniqueTeamHome),`uniqueTeamAway` = VALUES(uniqueTeamAway),`lastgoalteam` = VALUES(lastgoalteam),`servingplayer` = VALUES(servingplayer),`pointhome` = VALUES(pointhome),`pointaway` = VALUES(pointaway), `winner` = VALUES(winner), `winnername` = VALUES(winnername), `lastchangeby` = VALUES(lastchangeby),`xmltime` = VALUES(xmltime)");
									echo mysql_errno(),
									"<br>",
									mysql_error();
									...
									$checkrows = mysql_query("SELECT `matchid` FROM `ls_matches_volleyball` where `matchid` = '$matchId2db' Limit 1");
									echo mysql_errno(),
									"<br>",
									mysql_error();
