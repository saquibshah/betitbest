...
//HIER SCORING/SERVING ÄNDERN																		
								...
								$live_match_query = mysql_query("SELECT * FROM `sportnews_livescores_basketball` where `matchstatus` = '1st quarter' || `matchstatus` = '2nd quarter' || `matchstatus` = '3rd quarter' || `matchstatus` = '4th quarter' || `matchstatus` = 'Overtime'");
										
										while($row = mysql_fetch_array($live_match_query)){
											$match_id = $row['matchid'];
											if ($match_id!=$matchId2db){
												echo "LCA match ID:".$match_id."<br/>";
												//$status_update_query = mysql_query("UPDATE sportnews_livescores_basketball SET matchstatus='LCA' WHERE matchid='$match_id'");
											}
										}
										//end LCA check
								*/
