...
$xmlFile='../../xmls/sportradar/lct/livescore_multicast_betitbest_lct_4250.xml';
//$xmlFile='xmls/sportradar/lct_4250_(24-02-15).xml';
//echo time()."<br>";
$xmltime=0;
$sessionTime=-1;
$tournament="";
...
if (file_exists($xmlFile)) { 
	...
	$eintragen=date('d.M.Y-H:i:s',time())." - delta_xml Datei vorhanden.\nBeginne Verarbeitung..";
										$logname="/opt/users/www/betitbest-news/livescores/importer/log/tennis_import_main_xml2db.log";
	...
	if($doc){
		...
		foreach($xmlGeneratedTime as $xmlGeneratedTime) {
			...
			//read xml Counter
			$xmlcounter = $xmlGeneratedTime->getAttribute('counter');
		}
		...
		$query=mysql_query("SELECT counter FROM ls_matches_tennis LIMIT 1");
		$counterFromDB=mysql_fetch_array($query);
		$counterValueFromDB=$counterFromDB['counter'];
		echo "<br>Counter Value from XML: ".$xmlcounter;
					...
					foreach ($tournaments as $tournament) {
						...
						foreach ($matches as $match){
							...
							if(($match->getElementsByTagName( 'MatchDate' )->item(0)->nodeValue)!="" && ($homeTeam->item(0)->nodeValue)!="" && ($awayTeam->item(0)->nodeValue)!=""){
								...
								echo "<br>".$teamnamehome." - ".$teamnameaway;
		...
		//											$pointattr=$satzergebnis->Point[$countpoints]->attributes();
													...
													foreach ($score as $points){
														...
														if ($index==$countpoints){
															...
															$point=$points->getElementsByTagName( 'Point' )->item(0);
															//print_r($point);
															if ($point!=""){
																$lastscoring=$point->getAttribute('scoring');
																$actserving=$point->getAttribute('serving');
																$scorestring=$point->getAttribute('score');
																echo "<br>SCORE: ".$scorestring;
																echo "<br>SCORING: ".$lastscoring;
																echo "<br>SERVING: ".$actserving;
				...
				if ($status=="1st set"){
					...
					//$sessionTime=$sessionTime+1;
							...
							//check last changed by whom
							$lstChngQuery=mysql_query("SELECT lastchangeby FROM ls_matches_tennis WHERE matchid='$matchId'");
							$lstChngBy=mysql_fetch_array($lstChngQuery);
							$chngBy=$lstChngBy['lastchangeby'];
							echo "<br>Last changed by: ".$chngBy;
							...
							if($teamnamehome=="" || $teamnameaway=="" || $matchId2db<10){}
								...
								if($xmlcounter>$counterValueFromDB && $chngBy=="" || $xmlcounter>$counterValueFromDB && $chngBy=="main" || $xmlcounter>$counterValueFromDB && $chngBy=="delta"){
									mysql_query("INSERT INTO `ls_matches_tennis` (matchid, league, tournament, matchdate, matchstatus, hometeam, awayteam, scorehome, scoreaway, p1_scorehome, p1_scoreaway, p2_scorehome, p2_scoreaway, p3_scorehome, p3_scoreaway, p4_scorehome, p4_scoreaway, p5_scorehome, p5_scoreaway, uniqueTeamHome, uniqueTeamAway, lastgoalteam, servingplayer, pointhome, pointaway, lastchangeby, xmltime,counter) VALUES ('$matchId2db', '$category', '$tournamentname', '$m_timestamp','$status','$teamnamehome','$teamnameaway','$scorehome','$scoreaway', '$p1_home', '$p1_away', '$p2_home', '$p2_away', '$p3_home', '$p3_away', '$p4_home', '$p4_away', '$p5_home', '$p5_away','$uniqueTeamIdHome2db','$uniqueTeamIdAway2db', '$lastscoring', '$actserving', '$pointhome','$pointaway','main', $xmltime,'$counter') ON DUPLICATE KEY 
									UPDATE `matchid` = VALUES(matchid), `league` = VALUES(league), `tournament` = VALUES(tournament), `matchdate` = VALUES(matchdate), `matchstatus` = VALUES(matchstatus), `hometeam` = VALUES(hometeam), `awayteam` = VALUES(awayteam), `scorehome` = VALUES(scorehome),`scoreaway` = VALUES(scoreaway),`p1_scorehome` = VALUES(p1_scorehome),`p1_scoreaway` = VALUES(p1_scoreaway),`p2_scorehome` = VALUES(p2_scorehome),`p2_scoreaway` = VALUES(p2_scoreaway),`p3_scorehome` = VALUES(p3_scorehome),`p3_scoreaway` = VALUES(p3_scoreaway),`p4_scorehome` = VALUES(p4_scorehome),`p4_scoreaway` = VALUES(p4_scoreaway),`p5_scorehome` = VALUES(p5_scorehome),`p5_scoreaway` = VALUES(p5_scoreaway),`uniqueTeamHome` = VALUES(uniqueTeamHome),`uniqueTeamAway` = VALUES(uniqueTeamAway),`lastgoalteam` = VALUES(lastgoalteam),`servingplayer` = VALUES(servingplayer),`pointhome` = VALUES(pointhome),`pointaway` = VALUES(pointaway),`lastchangeby` = VALUES(lastchangeby),`xmltime` = VALUES(xmltime),`counter` = VALUES(counter)");
									echo mysql_errno(),
									"<br>",
									mysql_error();									
								...
								if ($num_rows<1 && $xmlcounter>$counterValueFromDB && $chngBy=="" || $num_rows<1 && $xmlcounter>$counterValueFromDB && $chngBy=="main" || $num_rows<1 && $xmlcounter>$counterValueFromDB && $chngBy=="delta"){
									mysql_query("INSERT INTO `ls_matches_tennis` (matchid, league, tournament, matchdate, matchstatus, hometeam, awayteam, scorehome, scoreaway, p1_scorehome, p1_scoreaway, p2_scorehome, p2_scoreaway, p3_scorehome, p3_scoreaway, p4_scorehome, p4_scoreaway, p5_scorehome, p5_scoreaway, uniqueTeamHome, uniqueTeamAway, lastgoalteam, servingplayer, pointhome, pointaway, lastchangeby, xmltime,counter) VALUES ('$matchId2db', '$category', '$tournamentname', '$m_timestamp','$status','$teamnamehome','$teamnameaway','$scorehome','$scoreaway', '$p1_home', '$p1_away', '$p2_home', '$p2_away', '$p3_home', '$p3_away', '$p4_home', '$p4_away', '$p5_home', '$p5_away','$uniqueTeamIdHome2db','$uniqueTeamIdAway2db', '$lastscoring', '$actserving', '$pointhome','$pointaway','main', $xmltime,'$counter') ON DUPLICATE KEY 
									UPDATE `matchid` = VALUES(matchid), `league` = VALUES(league), `tournament` = VALUES(tournament), `matchdate` = VALUES(matchdate), `matchstatus` = VALUES(matchstatus), `hometeam` = VALUES(hometeam), `awayteam` = VALUES(awayteam), `scorehome` = VALUES(scorehome),`scoreaway` = VALUES(scoreaway),`p1_scorehome` = VALUES(p1_scorehome),`p1_scoreaway` = VALUES(p1_scoreaway),`p2_scorehome` = VALUES(p2_scorehome),`p2_scoreaway` = VALUES(p2_scoreaway),`p3_scorehome` = VALUES(p3_scorehome),`p3_scoreaway` = VALUES(p3_scoreaway),`p4_scorehome` = VALUES(p4_scorehome),`p4_scoreaway` = VALUES(p4_scoreaway),`p5_scorehome` = VALUES(p5_scorehome),`p5_scoreaway` = VALUES(p5_scoreaway),`uniqueTeamHome` = VALUES(uniqueTeamHome),`uniqueTeamAway` = VALUES(uniqueTeamAway),`lastgoalteam` = VALUES(lastgoalteam),`servingplayer` = VALUES(servingplayer),`pointhome` = VALUES(pointhome),`pointaway` = VALUES(pointaway),`lastchangeby` = VALUES(lastchangeby),`xmltime` = VALUES(xmltime),`counter` = VALUES(counter)");
									echo mysql_errno(),
									"<br>",
									...
									mysql_query("INSERT INTO `ls_matches_tennis` (matchid, league, tournament, matchdate, matchstatus, hometeam, awayteam, scorehome, scoreaway, p1_scorehome, p1_scoreaway, p2_scorehome, p2_scoreaway, p3_scorehome, p3_scoreaway, p4_scorehome, p4_scoreaway, p5_scorehome, p5_scoreaway, uniqueTeamHome, uniqueTeamAway, lastgoalteam, servingplayer, pointhome, pointaway, lastchangeby, xmltime,counter) VALUES ('$matchId2db', '$country', '$league', '$m_timestamp','$status','$teamnamehome','$teamnameaway','$scorehome','$scoreaway', '$p1_home', '$p1_away', '$p2_home', '$p2_away', '$p3_home', '$p3_away', '$p4_home', '$p4_away', '$p5_home', '$p5_away','$uniqueTeamIdHome2db','$uniqueTeamIdAway2db', '$lastscoring', '$actserving', '$pointhome','$pointaway','delta', $xmltime,'$counter') ON DUPLICATE KEY 
									UPDATE `matchid` = VALUES(matchid), `league` = VALUES(league), `tournament` = VALUES(tournament), `matchdate` = VALUES(matchdate), `matchstatus` = VALUES(matchstatus), `hometeam` = VALUES(hometeam), `awayteam` = VALUES(awayteam), `scorehome` = VALUES(scorehome),`scoreaway` = VALUES(scoreaway),`p1_scorehome` = VALUES(p1_scorehome),`p1_scoreaway` = VALUES(p1_scoreaway),`p2_scorehome` = VALUES(p2_scorehome),`p2_scoreaway` = VALUES(p2_scoreaway),`p3_scorehome` = VALUES(p3_scorehome),`p3_scoreaway` = VALUES(p3_scoreaway),`p4_scorehome` = VALUES(p4_scorehome),`p4_scoreaway` = VALUES(p4_scoreaway),`p5_scorehome` = VALUES(p5_scorehome),`p5_scoreaway` = VALUES(p5_scoreaway),`uniqueTeamHome` = VALUES(uniqueTeamHome),`uniqueTeamAway` = VALUES(uniqueTeamAway),`lastgoalteam` = VALUES(lastgoalteam),`servingplayer` = VALUES(servingplayer),`pointhome` = VALUES(pointhome),`pointaway` = VALUES(pointaway),`lastchangeby` = VALUES(lastchangeby),`xmltime` = VALUES(xmltime),`counter` = VALUES(counter)");
									}
								else {
