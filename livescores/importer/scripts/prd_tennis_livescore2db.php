            ...
            $LigaId = $tournament->getAttribute('BetradarTournamentId');
						...
						foreach ($matches as $match){
							...
							if(($match->getElementsByTagName( 'MatchDate' )->item(0)->nodeValue)!="" && ($homeTeam->item(0)->nodeValue)!="" && ($awayTeam->item(0)->nodeValue)!=""){
								...
								//$winner=(simplexml_import_dom($match)->Winner->children());
...
//																					$pointattr=$satzergebnis->Point[$countpoints]->attributes();
				...
				if ($status=="1st set"){
					...
					//$sessionTime=$sessionTime+1;
							...
							$live_match_query = mysql_query("SELECT * FROM `ls_matches_tennis` where `matchstatus` = '1st set' || `matchstatus` = '2nd set' || `matchstatus` = '3rd set' || `matchstatus` = '4th set' || `matchstatus` = '5th set'");
										
										while($row = mysql_fetch_array($live_match_query)){
											$match_id = $row['matchid'];
											if ($match_id!=$matchId2db){
												echo "LCA match ID:".$match_id."<br/>";
												//$status_update_query = mysql_query("UPDATE ls_matches_tennis SET matchstatus='LCA' WHERE matchid='$match_id'");
											}
										}
										//end LCA check
							...
							if($teamnamehome=="" || $teamnameaway=="" || $matchId2db<10){}
								...
								if($counter>$counterValueFromDB){
									mysql_query("INSERT INTO `ls_matches_tennis` (matchid, leagueid, league, tournament, matchdate, matchstatus, hometeam, awayteam, scorehome, scoreaway, p1_scorehome, p1_scoreaway, p2_scorehome, p2_scoreaway, p3_scorehome, p3_scoreaway, p4_scorehome, p4_scoreaway, p5_scorehome, p5_scoreaway, uniqueTeamHome, uniqueTeamAway, lastgoalteam, servingplayer, winner, pointhome, pointaway, lastchangeby, xmltime, counter) VALUES ('$matchId2db', '$LigaId', '$category', '$tournamentname', '$m_timestamp','$status','$teamnamehome','$teamnameaway','$scorehome','$scoreaway', '$p1_home', '$p1_away', '$p2_home', '$p2_away', '$p3_home', '$p3_away', '$p4_home', '$p4_away', '$p5_home', '$p5_away','$uniqueTeamIdHome2db','$uniqueTeamIdAway2db', '$lastscoring', '$actserving', '$winner', '$pointhome','$pointaway','main', $xmltime,'$counter') ON DUPLICATE KEY
									UPDATE `leagueid` = VALUES(leagueid), `matchid` = VALUES(matchid), `league` = VALUES(league), `tournament` = VALUES(tournament), `matchdate` = VALUES(matchdate), `matchstatus` = VALUES(matchstatus), `hometeam` = VALUES(hometeam), `awayteam` = VALUES(awayteam), `scorehome` = VALUES(scorehome),`scoreaway` = VALUES(scoreaway),`p1_scorehome` = VALUES(p1_scorehome),`p1_scoreaway` = VALUES(p1_scoreaway),`p2_scorehome` = VALUES(p2_scorehome),`p2_scoreaway` = VALUES(p2_scoreaway),`p3_scorehome` = VALUES(p3_scorehome),`p3_scoreaway` = VALUES(p3_scoreaway),`p4_scorehome` = VALUES(p4_scorehome),`p4_scoreaway` = VALUES(p4_scoreaway),`p5_scorehome` = VALUES(p5_scorehome),`p5_scoreaway` = VALUES(p5_scoreaway),`uniqueTeamHome` = VALUES(uniqueTeamHome),`uniqueTeamAway` = VALUES(uniqueTeamAway),`lastgoalteam` = VALUES(lastgoalteam),`servingplayer` = VALUES(servingplayer),`winner` = VALUES(winner),`pointhome` = VALUES(pointhome),`pointaway` = VALUES(pointaway),`lastchangeby` = VALUES(lastchangeby),`xmltime` = VALUES(xmltime),`counter` = VALUES(counter)");
									echo mysql_errno(),
									"<br>",
									mysql_error();
