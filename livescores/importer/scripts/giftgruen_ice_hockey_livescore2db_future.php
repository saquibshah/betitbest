...
if (file_exists($xmlFile)) { 
	...
	$eintragen=date('d.M.Y-H:i:s',time())." - delta_xml Datei vorhanden.\nBeginne Verarbeitung..";
										$logname="/opt/users/www/betitbest-news/livescores/importer/log/ice_hockey_import_main_xml2db.log";
...
//			foreach($xml->Tournament as $sport) {
			...
			if ($sport->Name=="Ice Hockey" ){echo "<br>Sportart:Ice Hockey";
				foreach ($sport->children() as $category) {
						//echo "category: ";
					if($category->Name=="Germany" || $category->Name=="Czech" || $category->Name=="Sweden" || $category->Name=="Russia" || $category->Name=="USA"){
						...
						$UniqueTournamentId2db=$UniqueTournamentId['UniqueTournamentId'];
							if($LigaId['BetradarTournamentId']=="109" || $LigaId['BetradarTournamentId']=="93" || $LigaId['BetradarTournamentId']=="673" || $LigaId['BetradarTournamentId']=="115" || $LigaId['BetradarTournamentId']=="142" || $LigaId['BetradarTournamentId']=="6795" || $LigaId['BetradarTournamentId']=="10560"){
								...
								if($match->MatchDate[0]!="" && $match->Team1->Name[0]!="" && $match->Team2->Name[0]!=""){
									...
									//echo $match->Team1->Name[0]."</td><td class=\"ls_team2\">".$match->Team2->Name[0]."</td><td class=\"ls_status_result\">";							
										...
										echo "<br>Start: ".$match->MatchDate[0];
											...
											if($status=="Ended" || $status=="1st period" || $status=="2nd period" || $status=="3rd period" || $status=="1st extra" || $status=="2nd extra" || $status=="Overtime" || $status=="AET" || $status=="Pause" || $status=="Postponed" || $status=="AP" ){	
...
//HIER QUARTER ÄNDERN															
																...
																if ($scoreattr=="Period1")
																{
																	...
																	echo "<br>Period ".$c." -> ".$scores->Score[$c]->Team1." : ".$scores->Score[$c]->Team2;
																	}	
																	...
																	else if ($scoreattr=="Period4")
																{
																	...
																	else if ($scoreattr=="Ice Hockey")
																{	
																		echo "<br>Hier Ice Hockey<br>";
																		$lastscoring=0;
																		$actserving=0;
																		$scorestring=0;
																		$scorestringarray=0;
...
//HIER SCORING/SERVING ÄNDERN																		
												...
												//if($sessionTime>0){
														if ($status=="1st period"){
															echo "<br>1. Drittel";
														}
														else if ($status=="2nd period"){
															echo "<br>2. Drittel";
														}
														else if ($status=="3rd period"){
															echo "<br>3. Drittel";
								...
								if($LigaId=="109"){$league="Extraliga";}
								else if ($LigaId=="93"){$league="DEL";}
								else if ($LigaId=="673"){$league="KHL";}
								else if ($LigaId=="115"){$league="SHL";}
								else if ($LigaId=="142"){$league="NHL";}
								else if ($LigaId=="6795"){$league="NHL Preseason";}
								else if ($LigaId=="10560"){$league="NHL Playoffs";}
								...
								if($teamnamehome=="" || $teamnameaway=="" || $matchId2db<10){}
									...
									mysql_query("INSERT INTO `sportnews_livescores_ice_hockey` (matchid, leagueid, categoryid, uniquetournamentid, country, league, matchdate, matchstatus, hometeam, awayteam, uniqueTeamHome, uniqueTeamAway, scorehome, scoreaway, q1_scorehome, q1_scoreaway, q2_scorehome, q2_scoreaway, q3_scorehome, q3_scoreaway, ot_scorehome, ot_scoreaway, winner, winnername, lastteamscored, createtime, lastchangeby, xmltime) VALUES ('$matchId2db','$LigaId', '$CategoryId', '$UniqueTournamentId2db', '$country', '$league', '$m_timestamp','$status','$teamnamehome','$teamnameaway','$uniqueTeamIdHome2db','$uniqueTeamIdAway2db','$scorehome','$scoreaway', '$p1_home', '$p1_away', '$p2_home', '$p2_away', '$p3_home', '$p3_away', '$ot_home', '$ot_away', '$winner', '$winnername', '$lastscoring', '$jetzt', 'future', '$xmltime') ON DUPLICATE KEY 
									UPDATE `leagueid` = VALUES(leagueid), `categoryid` = VALUES(categoryid), `uniquetournamentid` = VALUES(uniquetournamentid), `country` = VALUES(country), `league` = VALUES(league), `matchdate` = VALUES(matchdate), `matchstatus` = VALUES(matchstatus), `hometeam` = VALUES(hometeam), `awayteam` = VALUES(awayteam),`uniqueTeamHome` = VALUES(uniqueTeamHome),`uniqueTeamAway` = VALUES(uniqueTeamAway), `scorehome` = VALUES(scorehome),`scoreaway` = VALUES(scoreaway),`q1_scorehome` = VALUES(q1_scorehome),`q1_scoreaway` = VALUES(q1_scoreaway),`q2_scorehome` = VALUES(q2_scorehome),`q2_scoreaway` = VALUES(q2_scoreaway),`q3_scorehome` = VALUES(q3_scorehome),`q3_scoreaway` = VALUES(q3_scoreaway),`ot_scorehome` = VALUES(ot_scorehome),`ot_scoreaway` = VALUES(ot_scoreaway), `winner` = VALUES(winner), `winnername` = VALUES(winnername), `lastteamscored` = VALUES(lastteamscored),`lastchangeby` = VALUES(lastchangeby), `xmltime` = VALUES(xmltime)");
									echo mysql_errno(),
									"<br>",
									mysql_error();									
									//$logname="main_xml2db.log";
										...
										//fclose($datei);
									/*	
									$checkrows = mysql_query("SELECT `matchid` FROM `sportnews_livescores_ice_hockey` where `matchid` = '$matchId2db' Limit 1");
									echo mysql_errno(),
									"<br>",
									mysql_error();
									...
									if ($num_rows<1){
									mysql_query("INSERT INTO `sportnews_livescores_ice_hockey` (matchid, leagueid, country, league, matchdate, matchstatus, hometeam, awayteam, uniqueTeamHome, uniqueTeamAway, scorehome, scoreaway, q1_scorehome, q1_scoreaway, q2_scorehome, q2_scoreaway, q3_scorehome, q3_scoreaway, ot_scorehome, ot_scoreaway, winner, winnername, lastteamscored, createtime, lastchangeby, xmltime) VALUES ('$matchId2db','$LigaId', '$country', '$league', '$m_timestamp','$status','$teamnamehome','$teamnameaway','$uniqueTeamIdHome2db','$uniqueTeamIdAway2db','$scorehome','$scoreaway', '$p1_home', '$p1_away', '$p2_home', '$p2_away', '$p3_home', '$p3_away', '$ot_home', '$ot_away', '$winner', '$winnername', '$lastscoring', '$jetzt', 'future', '$xmltime') ON DUPLICATE KEY 
