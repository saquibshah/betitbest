		...
		foreach( $sports as $sport ){
			...
			if ($name=="Basketball" ){
				...
				foreach ($catagoryNames as $categories){
					...
					//echo "category: ";
					if($category=="Germany" || $category=="Lithuania" || $category=="Sweden" || $category=="International" || $category=="Austria" || $category=="USA" || $category=="Poland" ){
						...
						$LigaId = $tournament->getAttribute('BetradarTournamentId');
							//print $LigaId;
							//$LigaId=$tournament;
							if($LigaId=="154" || $LigaId=="177" || $LigaId=="1537" || $LigaId=="1820" || $LigaId=="737" || $LigaId=="4805" || $LigaId=="1550" || $LigaId=="9154" || $LigaId=="3191" || $LigaId=="2451" ){
...
//HIER SCORING/SERVING ÄNDERN																		
								...
								if($teamnamehome=="" || $teamnameaway=="" || $matchId2db<10){}
									...
									//mysql_query("INSERT INTO `ls_matches_basketball_test` (matchid, country, league, matchdate, matchstatus, hometeam, awayteam, uniqueTeamHome, uniqueTeamAway, scorehome, scoreaway, q1_scorehome, q1_scoreaway, q2_scorehome, q2_scoreaway, q3_scorehome, q3_scoreaway, q4_scorehome, q4_scoreaway, ot_scorehome, ot_scoreaway, lastteamscored, createtime, lastchangeby) VALUES ('$matchId2db', '$country', '$league', '$m_timestamp','$status','$teamnamehome','$teamnameaway','$uniqueTeamIdHome2db','$uniqueTeamIdAway2db','$scorehome','$scoreaway', '$q1_home', '$q1_away', '$q2_home', '$q2_away', '$q3_home', '$q3_away', '$q4_home', '$q4_away', '$ot_home', '$ot_away', '$lastscoring', '$jetzt', 'main') 
										//$logname="main_xml2db.log";
										//$datei=fopen($logname,"a");
										//fputs($datei,"$eintragen\n\n");
										//fclose($datei);
										
									$checkrows = mysql_query("SELECT `matchid` FROM `ls_matches_basketball_test` where `matchid` = '$matchId2db' AND `lastchangeby`='delta' Limit 1");
									$num_rows = mysql_num_rows($checkrows);
									$row = mysql_fetch_array($checkrows);
									$row_matchstatus = isset($row['matchstatus']);
									
									if ($num_rows<1){
									mysql_query("INSERT INTO `ls_matches_basketball_test` (matchid, country, league, matchdate, matchstatus, hometeam, awayteam, uniqueTeamHome, uniqueTeamAway, scorehome, scoreaway, q1_scorehome, q1_scoreaway, q2_scorehome, q2_scoreaway, q3_scorehome, q3_scoreaway, q4_scorehome, q4_scoreaway, ot_scorehome, ot_scoreaway, winner, winnername, lastteamscored, createtime, lastchangeby) VALUES ('$matchId2db', '$country', '$league', '$m_timestamp','$status','$teamnamehome','$teamnameaway','$uniqueTeamIdHome2db','$uniqueTeamIdAway2db','$scorehome','$scoreaway', '$p1_home', '$p1_away', '$p2_home', '$p2_away', '$p3_home', '$p3_away', '$p4_home', '$p4_away', '$ot_home', '$ot_away', '$winner', '$winnername', '$lastscoring', '$jetzt', 'delta') ON DUPLICATE KEY 
									...
									else if($row_matchstatus=="Not started" && $status=="1st half" || $row_matchstatus=="1st half" && $status=="Halftime" || $row_matchstatus=="Halftime" && $status=="2nd half" || $row_matchstatus=="2nd half" && $status=="Ended" ){
									mysql_query("INSERT INTO `ls_matches_basketball_test` (matchid, country, league, matchdate, matchstatus, hometeam, awayteam, uniqueTeamHome, uniqueTeamAway, scorehome, scoreaway, q1_scorehome, q1_scoreaway, q2_scorehome, q2_scoreaway, q3_scorehome, q3_scoreaway, q4_scorehome, q4_scoreaway, ot_scorehome, ot_scoreaway, winner, winnername, lastteamscored, createtime, lastchangeby) VALUES ('$matchId2db', '$country', '$league', '$m_timestamp','$status','$teamnamehome','$teamnameaway','$uniqueTeamIdHome2db','$uniqueTeamIdAway2db','$scorehome','$scoreaway', '$p1_home', '$p1_away', '$p2_home', '$p2_away', '$p3_home', '$p3_away', '$p4_home', '$p4_away', '$ot_home', '$ot_away', '$winner', '$winnername', '$lastscoring', '$jetzt', 'delta') ON DUPLICATE KEY 
									UPDATE `country` = VALUES(country), `league` = VALUES(league), `matchdate` = VALUES(matchdate), `matchstatus` = VALUES(matchstatus), `hometeam` = VALUES(hometeam), `awayteam` = VALUES(awayteam),`uniqueTeamHome` = VALUES(uniqueTeamHome),`uniqueTeamAway` = VALUES(uniqueTeamAway), `scorehome` = VALUES(scorehome),`scoreaway` = VALUES(scoreaway),`q1_scorehome` = VALUES(q1_scorehome),`q1_scoreaway` = VALUES(q1_scoreaway),`q2_scorehome` = VALUES(q2_scorehome),`q2_scoreaway` = VALUES(q2_scoreaway),`q3_scorehome` = VALUES(q3_scorehome),`q3_scoreaway` = VALUES(q3_scoreaway),`q4_scorehome` = VALUES(q4_scorehome),`q4_scoreaway` = VALUES(q4_scoreaway),`ot_scorehome` = VALUES(ot_scorehome),`ot_scoreaway` = VALUES(ot_scoreaway), `winner` = VALUES(winner), `winnername` = VALUES(winnername), `lastteamscored` = VALUES(lastteamscored),`lastchangeby` = VALUES(lastchangeby)");
									}
									else {
