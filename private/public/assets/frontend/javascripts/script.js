...
(function () {
                ...
                if (data.type === 'news') {
                    ...
                    for (i = 0; i < data.items.length; ++i) {
                        ...
                        if ($('#news-item-' + data.items[i].uid).length === 0) {
                            ...
                            if (data.items[i].media_url !== null) {
    ...
    searchKeyListener = function (val) {
        ...
        url = window.location.protocol + '//' + window.location.host + '/' + language + '/search/' + val;
        $.ajax(url, {
            dataType: 'json'
        }).done(function (data) {
            ...
            if (data.team !== undefined || data.unique !== undefined || data.tournament !== undefined || data.category
                ...
                if( siteurl.indexOf("livescores") !== -1 ) {
                  preurl = "/livescores";
                }
                ...
                if (Object.keys(data.team).length) {
                    ...
                    for (item in data.team) {
                        if (data.team.hasOwnProperty(item)) {
                            result += '<li><a href="' + preurl + '/' + language + '/' + data.team[item].url + '">'
        ...
        $('.add-to-fav').click(function (ev) {
            ...
            if (cookieContent !== undefined) {
                ...
                if (cookieString.length > 0) {
                    ...
                    for (i = 0; i < cookieArray.length; ++i) {
                        if (cookieArray[i] === $(this).attr('data-options')) {
                            add = false;
                    ...
                    if (add) {
                        $.cookie("favorites", cookieString + ";" + $(this).attr('data-options'),
                            {path: '/', expires: 999});
                        $('span', this).html($('.add-to-fav').attr('data-removetext'));
                    } else {
                        $.cookie("favorites", cookieArray.join(";"), {path: '/', expires: 999});
                        $('span', this).html($('.add-to-fav').attr('data-default'));
                    }

                } else {

                    $.cookie("favorites", $(this).attr('data-options'), {path: '/', expires: 999});
