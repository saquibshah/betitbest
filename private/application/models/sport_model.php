...
class Sport_model extends CI_Model {
    ...
    public function activate($uid) {
        ...
        $this->db->update('sport', array('hidden' => 0));
    }

    public function deactivate($uid) {
        $this->db->where('uid', $uid);
        $this->db->update('sport', array('hidden' => 1));
    }
    ...
    public function get_categories($uid) {
        $this->db->select('*')->from('category')->where('sport_uid', $uid)->where('hidden', 0)->order_by('name', 'asc');
    ...
    public function get_db_videos($uid) {
        $this->db->select('*')
				...
				->order_by('videoitems.published', 'desc')->group_by('videoitems.uid')
                ->limit(15);
    ...
    public function set_headerimage($uid, $path) {
        $this->db->where('uid', $uid);
        $this->db->update('sport', array('header_image' => $path));
    }

    public function set_name($uid, $name) {
        $this->db->where('uid', $uid);
        $this->db->update('sport', array('name' => $name));
    }

    public function get_match_count($offsetTime, $severStartTimeOfDate){
        $dataReturn = array();
        $serverEndTimeOfNext3Date = $severStartTimeOfDate + 86400*3;
        $arrData = array(
            1 => 'sportnews_livescores_basketball',
            2 => 'sportnews_livescores_handball',
            3 => 'sportnews_livescores_ice_hockey',
        ...
        foreach ($arrData as $key => $val) {
            $r = $this->db->query("SELECT COUNT(*) AS total FROM {$val} WHERE (matchdate + {$offsetTime} >= '{$severStartTimeOfDate}' AND matchdate + {$offsetTime} <= '{$serverEndTimeOfNext3Date}') LIMIT 0,1")->result_array();
            if (!empty($r)) {
                $dataReturn['sport_'.$key] = intval($r[0]['total']);
            }
        }
        return $dataReturn;
    }
