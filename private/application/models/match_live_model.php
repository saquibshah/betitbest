...
class Match_live_model extends CI_Model {
    ...
    private $listCategoryConvertIDs = array();
    private $listTournament = array();
    private $listTournamentConvertIDs = array();
    ...
    private $displaySportLayouts = array(
        ...
        47 => array(
            'titles' => array('SET', 'LEG'),
            'fields_home' => array('scorehome', 'pointhome'),
            'fields_away' => array('scoreaway', 'pointaway')
        )
    );
    ...
    public function getTemplate() {
        if ($this->sportSeoURL == "tennis" || $this->sportSeoURL == "basketball" || $this->sportSeoURL == "ice-hockey" || $this->sportSeoURL == "football" || $this->sportSeoURL == "darts") {
    ...
    public function getListCategory() {
        $query = $this->db->query("SELECT uid, `betradar_uid`, `seourl` FROM `sportnews_category`");
        foreach ($query->result_array() as $row) {
            $this->listCategory[$row['betradar_uid']] = $row['seourl'];
            $this->listCategoryConvertIDs[$row['betradar_uid']] = $row['uid'];
        }
        return $this;
    }
    ...
    public function getTournament() {
        if ($this->sportSeoURL == "tennis") {
            $query = $this->db->query("SELECT uid, `betradar_uid`, `seourl` FROM `sportnews_tournament`");
            foreach ($query->result_array() as $row) {
                $this->listTournament[$row['betradar_uid']] = $row['seourl'];
                $this->listTournamentConvertIDs[$row['betradar_uid']] = $row['uid'];
            }
        } else {
            $query = $this->db->query("SELECT uid, `betradar_uid`, `seourl` FROM `sportnews_unique_tournament`");
            foreach ($query->result_array() as $row) {
                $this->listTournament[$row['betradar_uid']] = $row['seourl'];
                $this->listTournamentConvertIDs[$row['betradar_uid']] = $row['uid'];
            }
...
//        switch ($this->sport) {
//            case 8:
//                $this->joins = " INNER JOIN  sportnews_category ON {$this->table}.categoryid = sportnews_category.betradar_uid AND sportnews_category.hidden = 0 "
//                . " INNER JOIN sportnews_tournament ON {$this->table}.leagueid = sportnews_tournament.betradar_uid AND sportnews_tournament.hidden = 1 ";
//                break;
//            default:
//                $this->joins = " INNER JOIN  sportnews_category ON {$this->table}.categoryid = sportnews_category.betradar_uid AND sportnews_category.hidden = 0 "
//                . " INNER JOIN sportnews_unique_tournament ON {$this->table}.uniquetournamentid = sportnews_unique_tournament.betradar_uid AND sportnews_unique_tournament.hidden = 0 ";
//                break;
//        }
//        $this->joins = '';
    }
    ...
    public function get($limit = 100, $offset = 0, $sport = false, $category = false, $tournament = false, $team = false) {
        ...
        if (!$this->isLive) {
            $startTime = $this->input->get_post('startDateTime');
            $endTime = $this->input->get_post('endDateTime');
            
            if (empty($startTime) && $this->isFavorite) {
                $startTime = time() - 4*86400;
            }
            if (empty($endTime) && $this->isFavorite) {
                $endTime = time() + 4*86400;
            }
            $this->where .= " AND ((matchdate >= '{$startTime}' AND matchdate <= '{$endTime}') OR (matchstatus IN ('".implode("','", $this->liveStatus[$this->sport])."') AND matchdate >= '".(time() - 4*3600)."'))";
        } else {
            
            $this->where .= " AND matchstatus IN ('".implode("','", $this->liveStatus[$this->sport])."') AND matchdate >= '".(time() - 4*3600)."'";
        }
        
        $query = $this->db->query("SELECT {$this->table}.* FROM `{$this->table}`{$this->joins}" . $this->where . " ORDER BY `matchdate` ASC");
        $i = 0;
        foreach ($query->result_array() as $row) {
    ...
    private function generateLayoutForEachSport(&$row) {
        if ($this->sport == 8 || $this->sport == 1 || $this->sport == 6 || $this->sport == 3 || $this->sport == 47) {
    ...
    function formatDataBeforeReturn($data) {
        if (($this->sport == 8 || $this->sport == 1 || $this->sport == 6 || $this->sport == 3 || $this->sport == 47) && $this->input->get_post('autoload') === FALSE && $this->input->get_post('isFavourite') === FALSE) {
    ...
    public function getCalendarMatchesCount($offset = 0){
        
        $this->generateSQLForLive();
        
        $startTime = strtotime(date('Y-m-d 00:00:00', strtotime('-7 days')));
        $endTime = strtotime(date('Y-m-d 23:59:59', strtotime('+7 days')));
        
        $temp = $startTime;
        $dataReturn = array();
        while ($temp < $endTime) {
            $dataReturn[date('Ymd', $temp)] = 0;
            $temp += 86400;
        }
        ...
        $this->where .= " AND (matchdate + {$offset} >= '{$startTime}' AND matchdate + {$offset} <= '{$endTime}') ";
        
        $query = $this->db->query("SELECT DATE(FROM_UNIXTIME(matchdate + {$offset})) AS `mdate`, COUNT(*) AS `matches_total` FROM `{$this->table}`" . $this->where . " GROUP BY DATE(FROM_UNIXTIME(matchdate + {$offset})) ORDER BY `mdate` ASC");
        
        foreach ($query->result_array() as $row) {
            $index = date('Ymd', strtotime($row['mdate']));
            $dataReturn[$index] = $row['matches_total'];
        }
