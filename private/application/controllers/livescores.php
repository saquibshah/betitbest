...
class Livescores extends CI_Controller {
    ...
    public function _remap() {
        ...
        if ($segs[2] == 'matchcount' || $segs[1] == 'matchcount') {
            $this->matchcount();
            exit;
        }
        ...
        if ($this->input->get_post("ajax") == "yes") {
            ...
            if ($this->input->get('matchcount') == "yes") {
                $timeLocal = intval($this->input->get('time'));
                $date = $this->input->get('date');
                $month = $this->input->get('month');
                $year = $this->input->get('year');
                $offsetTime = strtotime("{$year}-{$month}-{$date} 00:00:00") - $timeLocal;
                echo json_encode($this->match->getCalendarMatchesCount($offsetTime));
                exit;
            } else {
                echo json_encode($this->match->get(1000));
                exit;
            }
        }
        ...
        if ($tournament !== false) {
            ...
            if ($tournament['tntype'] == 'tournament') {
                ...
                if (strlen($tournament['header_image']) > 0) {
                    ...
                    $headerdata['headerImage'] = 'pool/uploads/unique_tournament/' . $tournament['header_image'];
                }
            }
    ...
    public function matchcount(){
        ...
        $startTimeOfDate = $this->input->get('startTimeOfDate');
        $timeZoneOffset = $this->input->get('timeZoneOffset');
        $dateLocal = $this->input->get('date');
        $severStartTimeOfDate = strtotime("{$dateLocal['year']}-{$dateLocal['month']}-{$dateLocal['date']} 00:00:00");
        
        $cachedCount = null;
        $cachedCount = $this->cache->get($timeZoneOffset."_".date('Ymd')."_matchescount");
        if (!$cachedCount) {
            $offsetTime = $severStartTimeOfDate - $startTimeOfDate;
            $cachedCount = $this->sport->get_match_count($offsetTime, $severStartTimeOfDate);
            $this->cache->write($cachedCount, $timeZoneOffset."_".date('Ymd')."_matchescount", 3600);
        }
        echo json_encode($cachedCount);
        exit;
    }
}
